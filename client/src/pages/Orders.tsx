...
import { IOrderTypes, IProductProps } from "../types";
...
const Orders = () => {
  ...
  // Function to split the paymentId into two lines
  const splitPaymentId = (paymentId: string) => {
    const midPoint = Math.ceil(paymentId.length / 2);
    return [paymentId.slice(0, midPoint), paymentId.slice(midPoint)];
  };
          ...
          <div className="flex flex-col gap-3">
            <div className="space-y-6 divide-y divide-gray-900/10">
              {orders?.map((order: IOrderTypes) => {
                ...
                const paymentIdParts = order?.paymentId
                  ? splitPaymentId(order?.paymentId)
                  : ["", ""];
                      ...
                      <>
                        ...
                        <DisclosurePanel as="dd" className="mt-5 pr-12">
                          <div className="flex flex-col gap-2 bg-[#f4f4f480] p-5 border border-gray-200">
                            ...
                            {order?.orderItems?.map((item: IProductProps) => (
                              <div
                                key={item?._id}
                                className="flex flex-col sm:flex-row gap-6 border-b border-gray-200 py-3"
                              >
                                <Link
                                  to={`/product/${item?._id}`}
